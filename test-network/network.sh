...
function createOrgs() {
  ...
  docker-compose -f $COMPOSE_FILE_APP up -d 2>&1
}
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_APP down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/npci/msp organizations/fabric-ca/npci/tls-cert.pem organizations/fabric-ca/npci/ca-cert.pem organizations/fabric-ca/npci/IssuerPublicKey organizations/fabric-ca/npci/IssuerRevocationPublicKey organizations/fabric-ca/npci/fabric-ca-server.db'
...
# hdfcapp compose file
COMPOSE_FILE_APP=docker/docker-compose-app.yaml
