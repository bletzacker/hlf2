...
async function readUserPrivateDetails(userId) {
	try {
		...
		try {
            ...
            contract.addDiscoveryInterest({ name: chaincodeName, collectionNames: ['SBIMSPUserCollection'] });

            let result;
    
            console.log('\n**************** As SBI Client ****************');
            console.log('\n--> Evaluate Transaction: ReadUserFromCollection');
            result = await contract.evaluateTransaction('ReadUserFromCollection', 'SBIMSPUserCollection', `${userId}@SBI`);
